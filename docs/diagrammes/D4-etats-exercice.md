...
stateDiagram-v2
    ...
    cloture --> EN_ATTENTE_RELECTURE : au moins un etudiant present<br/>autre que l'auteur — RG14, RG19
    cloture --> NON_ATTRIBUABLE : aucun relecteur eligible<br/>RG17

    EN_ATTENTE_RELECTURE --> RELU : relecture rendue<br/>POST /api/relectures/{id} — 200 — RG18
    RELU --> RELU : correction avant finalisation<br/>POST /api/relectures/{id} — 200 — RG20

    EN_ATTENTE_RELECTURE --> EN_ATTENTE_RELECTURE : relecture jamais rendue<br/>visible au tableau — RG23
    ...
    finalisation --> RELU_FIGE : relecture rendue
    finalisation --> NON_RELU : relecture jamais rendue
...
| `RELU` | Une note et un commentaire ont été rendus. La relecture reste corrigible tant que la session n'est pas finalisée (`RG20`) | `RELU` |
...
| `RELU_FIGE` · `NON_RELU` | États post-finalisation. Ils ne sont pas stockés dans `exercices.statut` : ils résultent de la combinaison du statut de l'exercice et de `sessions.statut = FINALISEE`. Le tableau du formateur les distingue | dérivé |
...
- **La boucle sur `RELU`** matérialise `RG20` : la relecture est corrigible après avoir été rendue. C'est l'arbitrage `Q10` contre `Q15`, lisible sur le diagramme.
...
**Règles de gestion illustrées :** `RG8`, `RG9`, `RG10`, `RG13`, `RG14`, `RG17`, `RG18`, `RG19`, `RG20`, `RG21`, `RG23`.
